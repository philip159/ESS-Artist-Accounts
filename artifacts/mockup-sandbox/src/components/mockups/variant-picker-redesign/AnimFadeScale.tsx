...
function AddonCard({ label, price, desc, specs, selected, onClick, icon, delay }: any) {
  return (
...
export function AnimFadeScale() {
  ...
  const showContent = open || animating;
  const isExpanded = open && !animating;
  const isCollapsing = !open && animating;

  return (
    <div style={{ width: 480, margin: "0 auto", padding: 32, fontFamily: "system-ui, -apple-system, sans-serif" }}>
      <div style={{ marginBottom: 12, textAlign: "center" }}>
        <span style={{ fontSize: 11, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.08em", color: SUB }}>B. Fade + Scale</span>
        <p style={{ fontSize: 11, color: FAINT, marginTop: 4 }}>Cards fade in with a subtle scale-up. Elegant, Apple-inspired feel.</p>
      </div>

      <div style={{ border: `1px solid ${BORDER}`, borderRadius: R, overflow: "hidden" }}>
        <button type="button" onClick={handleToggle} className="w-full flex items-center justify-between" style={{
        ...
        {showContent && (
          <div style={{
            maxHeight: isCollapsing ? 0 : height + 40,
            overflow: "hidden",
            ...
            <div ref={contentRef} style={{
              ...
              opacity: isCollapsing ? 0 : (isExpanded ? 1 : 0),
              transform: isCollapsing ? "scale(0.97)" : (isExpanded ? "scale(1)" : "scale(0.97)"),
