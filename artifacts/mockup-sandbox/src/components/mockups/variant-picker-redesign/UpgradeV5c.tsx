...
function SpecDot({ label }: { label: string }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
      <div style={{ width: 4, height: 4, borderRadius: "50%", background: "rgba(26,26,26,0.2)", flexShrink: 0 }} />
      <span style={{ fontSize: 10.5, color: FAINT }}>{label}</span>
    </div>
  );
}

export function UpgradeV5c() {
    ...
    <div className="min-h-screen p-5" style={{ fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif', color: TEXT, background: "#fff" }}>
      <div className="max-w-[440px] mx-auto flex flex-col" style={{ gap: 12 }}>
        <div style={{ marginBottom: 4 }}>
          <span style={{ fontWeight: 700, fontSize: 14, letterSpacing: "-0.01em" }}>V5c: Detail + Bullet Specs</span>
          <p style={{ fontSize: 11, color: SUB, marginTop: 2 }}>Rich description with a clean dot-separated spec list underneath</p>
        </div>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 14, padding: "14px 14px 10px" }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <span style={{ fontSize: 11.5, color: SUB, lineHeight: 1.45, display: "block" }}>
                Nano-structured acrylic that virtually eliminates reflections. See your artwork exactly as the artist intended — zero glare, total clarity.
              </span>
            </div>
            <Checkbox selected={glazeSelected} />
          </div>
          <div style={{
            padding: "6px 14px 12px", display: "flex", gap: 14, flexWrap: "wrap",
          }}>
            <SpecDot label="99% UV protection" />
            <SpecDot label="Anti-reflective" />
            <SpecDot label="Shatter-resistant" />
            <SpecDot label="Lighter than glass" />
          </div>
        </button>
        ...
        <button type="button" onClick={() => setFrameSelected(!frameSelected)} style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 14, padding: "14px 14px 10px" }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <span style={{ fontSize: 11.5, color: SUB, lineHeight: 1.45, display: "block" }}>
                Hand-stained solid Ash with a deep shadow-box profile. Creates the floating effect seen in London's finest galleries.
              </span>
            </div>
          ...
          <div style={{
            ...
            <SpecDot label="Solid Ash hardwood" />
            <SpecDot label="45mm deep profile" />
            <SpecDot label="Hand-stained finish" />
          </div>
        </button>
      </div>
    </div>
