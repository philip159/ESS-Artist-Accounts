...
function MountSwatch({ label, selected, onClick, hasMount }: { label: string; selected: boolean; onClick: () => void; hasMount: boolean }) {
  return (
    <button type="button" onClick={onClick} style={{ width: 52, height: 40, borderRadius: 6, border: "none", outline: "none", padding: 0, cursor: "pointer", overflow: "hidden", position: "relative", background: "#fff" }}>
      <div style={{ position: "absolute", inset: 0, borderRadius: 6, border: selected ? `1.5px solid ${TEXT}` : `1px solid ${BL}`, pointerEvents: "none", zIndex: 2 }} />
      <div style={{ width: "100%", height: "100%", display: "flex", alignItems: "center", justifyContent: "center", background: "#fafafa" }}>
        {hasMount ? (
          <div style={{ position: "relative", width: 22, height: 28 }}>
            <div style={{ position: "absolute", inset: 0, background: "#fff", border: "1px solid rgba(26,26,26,0.1)", borderRadius: 1 }} />
            <div style={{ position: "absolute", top: 3, left: 3, right: 3, bottom: 3, background: "rgba(26,26,26,0.06)" }} />
          </div>
        ) : (
          <div style={{ width: 18, height: 24, background: "rgba(26,26,26,0.06)", border: "1px solid rgba(26,26,26,0.1)", borderRadius: 1 }} />
        )}
      </div>
    </button>
  );
    ...
    <button type="button" onClick={onClick} className="w-full text-left" style={{ padding: 0, borderRadius: R, border: `1px solid ${checked ? TEXT : BL}`, background: "#fff", cursor: "pointer", fontFamily: "inherit", transition: "all 0.2s", overflow: "hidden" }}>
      ...
      <div style={{ margin: "8px 12px 0", padding: "8px 0 10px", borderTop: "1px solid rgba(26,26,26,0.06)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div className="flex items-center" style={{ gap: 4 }}>
          ...
          <span style={{ fontSize: 10.5, color: FAINT }}>{details}</span>
        </div>
  ...
  return (
    <div className="min-h-screen p-5 sm:p-6" style={{ fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif', color: TEXT, background: "#fff" }}>
      <div className="max-w-[440px] mx-auto flex flex-col" style={{ gap: 0 }}>
        ...
        {/* TYPE — segmented tabs at the top */}
        <div className="flex" style={{ marginBottom: 20 }}>
          {(["print", "canvas"] as ProductType[]).map(t => (
            <button key={t} type="button" onClick={() => setProductType(t)} style={{
              flex: 1, height: 42, border: `1px solid ${productType === t ? TEXT : BORDER}`,
              borderRadius: t === "print" ? `${R}px 0 0 ${R}px` : `0 ${R}px ${R}px 0`,
              background: productType === t ? TEXT : "#fff",
              color: productType === t ? "#fff" : SUB,
              fontWeight: productType === t ? 500 : 400, fontSize: 14,
              cursor: "pointer", fontFamily: "inherit",
              marginLeft: t === "canvas" ? -1 : 0,
              position: "relative", zIndex: productType === t ? 2 : 1,
            }}>
              {t === "print" ? "Print" : "Canvas"}
            </button>
          ))}
        </div>
        ...
        {/* FRAME + MOUNT — on one line with a separator */}
        {!isCanvas && (
          <div style={{ marginBottom: 16 }}>
            <div className="flex items-baseline gap-2" style={{ marginBottom: 6 }}>
              <span style={{ fontWeight: 600, fontSize: 13 }}>Frame:</span>
              <span style={{ fontSize: 13 }}>{FRAMES.find(f => f.id === selectedFrame)?.label}</span>
              {mountSelected && <span style={{ fontSize: 12, color: SUB }}>+ Mount</span>}
            </div>
            <div className="flex items-center flex-wrap" style={{ gap: 8 }}>
              {FRAMES.map(f => <FrameSwatch key={f.id} frame={f} selected={selectedFrame === f.id} onClick={() => setSelectedFrame(f.id)} />)}
              <div style={{ width: 1, height: 24, background: "rgba(26,26,26,0.1)", margin: "0 2px" }} />
              <MountSwatch label="Yes" selected={mountSelected} onClick={() => setMountSelected(true)} hasMount />
              <MountSwatch label="No" selected={!mountSelected} onClick={() => setMountSelected(false)} hasMount={false} />
            </div>
          </div>
        )}
        ...
        <div style={{ marginBottom: 20 }}>
          <div className="flex items-baseline gap-2" style={{ marginBottom: 6 }}>
            <span style={{ fontWeight: 600, fontSize: 13 }}>Upgrades:</span>
            <span style={{ fontSize: 13, color: upgradeCount > 0 ? TEXT : SUB }}>{upgradeCount > 0 ? [glazeSelected && "Museum Glaze", upgradeSelected && !isCanvas && (isFramed ? "Box Frame" : "Photo Rag")].filter(Boolean).join(", ") : "None"}</span>
          </div>
          <div style={{ border: `1px solid ${BORDER}`, borderRadius: R, overflow: "hidden" }}>
            <button type="button" onClick={() => setUpgradesOpen(!upgradesOpen)} className="w-full flex items-center justify-between" style={{ height: 42, padding: "0 14px", background: "#fff", border: 0, cursor: "pointer", fontFamily: "inherit", color: upgradeCount > 0 ? TEXT : SUB, fontSize: 13 }}>
              <div className="flex items-center" style={{ gap: 6 }}>
                <span>{upgradeCount > 0 ? `${upgradeCount} upgrade${upgradeCount > 1 ? "s" : ""} selected` : "Add premium upgrades"}</span>
                {upgradeCount > 0 && !upgradesOpen && <span style={{ fontSize: 10, fontWeight: 600, background: TEXT, color: "#fff", borderRadius: 8, padding: "1px 6px" }}>{upgradeCount}</span>}
              </div>
              <ChevronDown size={14} style={{ opacity: 0.5, transform: upgradesOpen ? "rotate(180deg)" : "none", transition: "transform 0.2s" }} />
            </button>
            {upgradesOpen && (
              <div style={{ padding: 10, display: "flex", flexDirection: "column", gap: 8, borderTop: "1px solid rgba(26,26,26,0.06)" }}>
                <PremiumUpgradeCard title="Museum Quality Glaze" price="+ £45" headline="Anti-reflective, UV-filtering museum glass. Preserves colours 100+ years." details="Used by the National Gallery" checked={glazeSelected} onClick={() => setGlazeSelected(!glazeSelected)} icon={<Shield size={11} />} thumbnail={<div style={{ width: 40, height: 40, borderRadius: 5, background: "linear-gradient(145deg, rgba(200,220,240,0.3), rgba(255,255,255,0.5))", border: "1px solid rgba(26,26,26,0.08)", display: "flex", alignItems: "center", justifyContent: "center", position: "relative", flexShrink: 0 }}><div style={{ width: 18, height: 24, borderRadius: 1, background: "linear-gradient(180deg, rgba(255,255,255,0.9), rgba(240,245,250,0.8))", border: "1px solid rgba(26,26,26,0.06)" }} /></div>} />
                {!isCanvas && <PremiumUpgradeCard title={isFramed ? "Black Box Frame" : "Hahnemuhle Photo Rag"} price={isFramed ? "+ £40" : "+ £15"} headline={isFramed ? "Solid Ash, deep 32mm profile. Hand-stained and lacquered." : "308gsm 100% cotton rag. The gold standard for fine art."} details={isFramed ? "Handcrafted in London" : "Trusted by galleries since 1584"} checked={upgradeSelected} onClick={() => setUpgradeSelected(!upgradeSelected)} icon={<Gem size={11} />} thumbnail={<div style={{ width: 40, height: 40, borderRadius: 5, border: "1px solid rgba(26,26,26,0.08)", background: isFramed ? "linear-gradient(135deg, #2a2a2a, #1a1a1a, #333)" : "linear-gradient(145deg, #faf8f5, #ede5d8)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}><div style={{ width: isFramed ? 14 : 18, height: isFramed ? 18 : 24, border: isFramed ? "2px solid #444" : "1px solid rgba(26,26,26,0.08)", borderRadius: 1, background: isFramed ? "rgba(255,255,255,0.06)" : "rgba(255,255,255,0.7)" }} /></div>} />}
              </div>
            )}
          </div>
        </div>
