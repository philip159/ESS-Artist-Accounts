...
function AddonCard({ label, price, desc, specs, selected, onClick, icon, animStyle }: any) {
  return (
    <button type="button" onClick={onClick} style={{
      ...
      cursor: "pointer", textAlign: "left", fontFamily: "inherit", transition: "border-color 0.2s, background 0.2s",
      ...animStyle,
    }}>
...
export function AnimStagger() {
  ...
  const [cardStates, setCardStates] = useState([false, false]);
  ...
  useEffect(() => {
    if (open) {
      setCardStates([false, false]);
      setTimeout(() => setCardStates([true, false]), 80);
      setTimeout(() => setCardStates([true, true]), 200);
    } else {
      setCardStates([false, false]);
    }
  }, [open]);
  ...
  const getCardStyle = (idx: number) => ({
    opacity: cardStates[idx] ? 1 : 0,
    transform: cardStates[idx] ? "translateY(0)" : "translateY(12px)",
  ...
  return (
    <div style={{ width: 480, margin: "0 auto", padding: 32, fontFamily: "system-ui, -apple-system, sans-serif" }}>
      <div style={{ marginBottom: 12, textAlign: "center" }}>
        <span style={{ fontSize: 11, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.08em", color: SUB }}>C. Staggered Reveal</span>
        <p style={{ fontSize: 11, color: FAINT, marginTop: 4 }}>Each card slides up with a staggered delay. Polished, sequential entrance.</p>
      </div>
      ...
      <div style={{ border: `1px solid ${BORDER}`, borderRadius: R, overflow: "hidden" }}>
        <button type="button" onClick={() => setOpen(!open)} className="w-full flex items-center justify-between" style={{
          ...
          <ChevronDown size={14} style={{ opacity: 0.5, transform: open ? "rotate(180deg)" : "none", transition: "transform 0.3s ease" }} />
        </button>
        <div style={{
          ...
          <div ref={contentRef} style={{ padding: 12, display: "flex", flexDirection: "column", gap: 10, borderTop: `1px solid rgba(26,26,26,0.06)` }}>
            <AddonCard animStyle={getCardStyle(0)} label="Museum Grade Glaze" price="+ £45" desc="Nano-structured acrylic. Virtually eliminates reflections." specs={["99% UV protection", "<1% reflection", "Shatter-resistant"]} selected={glaze} onClick={() => setGlaze(!glaze)} icon={<Shield size={20} strokeWidth={1.5} style={{ color: "rgba(26,26,26,0.5)" }} />} />
            <AddonCard animStyle={getCardStyle(1)} label="Black Box Frame" price="+ £90" desc="Hand-stained solid Ash with a deep shadow-box profile." specs={["Solid Ash hardwood", "45mm deep profile", "Hand-stained"]} selected={frame} onClick={() => setFrame(!frame)} icon={<div style={{ width: 18, height: 22, border: "3px solid #555", borderRadius: 1, background: "rgba(255,255,255,0.06)" }} />} />
          </div>
        </div>
      </div>
    </div>
