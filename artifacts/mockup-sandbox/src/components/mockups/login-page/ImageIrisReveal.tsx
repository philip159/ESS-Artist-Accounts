...
const SLIDES = [
  ...
  { src: "/__mockup/images/ess-studio-02.jpg", pos: "center 30%",   title: "Gallery Wall",           caption: "Lauren Leely · East Side Studio · 2024" },
...
function IrisPanel({ ready }: { ready: boolean }) {
  ...
  useEffect(() => {
    const id = setInterval(() => {
      const n = (active + 1) % SLIDES.length;
      setNext(n);
      setRevealing(true);
      setTimeout(() => { setActive(n); setRevealing(false); }, 900);
    }, 5000);
    return () => clearInterval(id);
  }, [active]);

  return (
    <div className="relative hidden h-full w-[54%] overflow-hidden lg:block" style={{ background: "#111" }}>
      {/* Base layer — current image */}
      <img src={SLIDES[active].src} alt="" style={{ position:"absolute", inset:0, width:"100%", height:"100%", objectFit:"cover", objectPosition: SLIDES[active].pos }} />
      <div style={{ position:"absolute", inset:0, background:"rgba(0,0,0,0.3)" }} />

      {/* Iris layer — next image revealed via expanding circle */}
      {revealing && (
        <div
          style={{
            position: "absolute",
            inset: 0,
            overflow: "hidden",
            animation: "irisExpand 0.9s cubic-bezier(0.4,0,0.2,1) forwards",
          }}
        >
          <img src={SLIDES[next].src} alt="" style={{ width:"100%", height:"100%", objectFit:"cover", objectPosition: SLIDES[next].pos }} />
          <div style={{ position:"absolute", inset:0, background:"rgba(0,0,0,0.3)" }} />
        </div>
      )}

      {/* Gradient overlays */}
      <div className="pointer-events-none absolute inset-x-0 top-0" style={{ height:"30%", background:"linear-gradient(to bottom, rgba(0,0,0,0.5) 0%, transparent 100%)", zIndex:10 }} />
      <div className="pointer-events-none absolute inset-x-0 bottom-0" style={{ height:"45%", background:"linear-gradient(to top, rgba(0,0,0,0.8) 0%, rgba(0,0,0,0.2) 60%, transparent 100%)", zIndex:10 }} />

      {/* Badge */}
      <div className="absolute left-7 top-7 z-20">
        <span style={{ background:"rgba(255,255,255,0.12)", border:"1px solid rgba(255,255,255,0.18)", backdropFilter:"blur(10px)", borderRadius:"999px", padding:"5px 14px", fontSize:"10px", fontWeight:600, letterSpacing:"0.22em", textTransform:"uppercase", color:"rgba(255,255,255,0.85)" }}>
          The Collection
        </span>
      </div>
      ...
      <div className="absolute right-6 top-1/2 z-20 flex -translate-y-1/2 flex-col gap-2">
        {SLIDES.map((_,i) => (
          <button key={i} onClick={() => { setNext(i); setRevealing(true); setTimeout(()=>{setActive(i);setRevealing(false);},900); }}
      ...
      {/* Metadata */}
      <div className="absolute bottom-7 left-7 right-10 z-20">
        <p style={{ fontSize:"10px", fontWeight:600, letterSpacing:"0.22em", textTransform:"uppercase", color:"rgba(255,255,255,0.45)", marginBottom:"4px" }}>
      ...
      {!ready && <div style={{ position:"absolute", inset:0, background:"#111", zIndex:30, opacity:ready?0:1, transition:"opacity 0.8s ease" }} />}
    </div>
  );
}
...
export function ImageIrisReveal() {
