    ...
    <div className="hidden h-full w-[54%] lg:flex" style={{ gap: "3px", background: "#fff" }}>
      ...
      >
        <img
          src={IMGS[0].src}
          alt=""
          style={{ width: "100%", height: "100%", objectFit: "cover", objectPosition: IMGS[0].pos,
            animation: "kbMosaicA 12s ease-in-out infinite alternate" }}
        />
        ...
        {[1, 2].map((i) => (
          ...
          >
            <img
              src={IMGS[i].src}
              alt=""
              style={{ width: "100%", height: "100%", objectFit: "cover", objectPosition: IMGS[i].pos,
                animation: `kbMosaicB${i} 14s ease-in-out infinite alternate` }}
            />
          ...
          {mode === "signin" ? (
            <form onSubmit={e=>{e.preventDefault();setLoading(true);setTimeout(()=>setLoading(false),1400);}} className="space-y-3.5">
              ...
              <Field label="Password" icon={<Lock size={14}/>} type="password" placeholder="••••••••" value={password} onChange={setPassword}
                right={<button type="button" className="text-[10px] font-medium text-stone-400 underline-offset-2 hover:underline">Forgot?</button>}/>
